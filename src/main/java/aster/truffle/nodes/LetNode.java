...
public abstract class LetNode extends AsterExpressionNode {
  ...
  protected boolean writeBoolean(VirtualFrame frame, boolean value) {
    Profiler.inc("let_boolean");
    if (frame == null) {
      throw new RuntimeException(ErrorMessages.variableNotInitialized(name));
    }
    frame.setBoolean(slotIndex, value);
    if (AsterConfig.DEBUG) {
      System.err.println("DEBUG: let " + name + "=" + value + " @slot " + slotIndex + " (boolean)");
    }
    return value;
  }
  ...
  protected String writeString(VirtualFrame frame, String value) {
    Profiler.inc("let_string");
    if (frame == null) {
      throw new RuntimeException(ErrorMessages.variableNotInitialized(name));
    }
    frame.setObject(slotIndex, value);
    if (AsterConfig.DEBUG) {
      System.err.println("DEBUG: let " + name + "=" + value + " @slot " + slotIndex + " (String)");
    }
    return value;
  }
